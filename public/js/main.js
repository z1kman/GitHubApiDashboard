...
function SetSearchQuery() { //установка запроса 
    const LabelContainer = document.getElementById('LabelContainer'); //надпись у контейнера
    const SearchInput = document.getElementById('SearchInput'); //поле поиска
    //url для топ 10 репозиториев
    let url = `https://api.github.com/search/repositories?q=stars:>=10000&` +
        `sort=stars&order=desc&page=${NumberOfPage}&per_page=10&${AuthKey}`;
    //проверка на наличие символов в поисковой строке
    if (SearchInput.value != '') {
        //url для запроса из поисковой строки
        url = `https://api.github.com/search/repositories?q=${SearchInput.value}` +
            `&sort=stars&order=desc&page=${NumberOfPage}&per_page=10&${AuthKey}`;
    }
...
function CreateReposList(Response, Result) { //создание списка репозиториев
    const LabelContainer = document.getElementById('LabelContainer');
    const ReposList = document.getElementById('ReposList'); //список репозиториев
    const HeadersLink = Response.headers.get('link'); //заголовок ответа
    let MaxPageCount = 1;
    ...
    for (let i = 0; i < Result.items.length; i++) {
        ...
        LeftSide.innerHTML = `<div class = 'NameRepos' Link = '${Result.items[i].url}'` +
            `onclick='OpenRepos(this)'>${Result.items[i].name}</div>` +
...
async function CheckLastCommit(url) { //поиск последнего commit(а)
    try {
        const response = await fetch(url + "?per_page=1&" + AuthKey);
        const result = await response.json();
...
function GoToPage(btn) { //переход по страницам
    ...
    const BtnFlag = document.getElementsByClassName('ActivePage');
